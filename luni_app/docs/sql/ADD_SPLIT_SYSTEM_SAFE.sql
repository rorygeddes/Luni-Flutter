-- Add Split System: Groups, Members, and Split Transactions (SAFE VERSION)
...
DROP TABLE IF EXISTS split_transactions CASCADE;
...
-- 3. Create group_members table (many-to-many relationship)
...
-- 4. Create split_transactions table (tracks who owes what)
...
-- 5. Create split_participants table (individual split details)
...
-- 9. RLS Policies for split_transactions
...
-- 10. RLS Policies for split_participants
...
CREATE INDEX idx_split_transactions_payer ON split_transactions(payer_id);
CREATE INDEX idx_split_transactions_transaction ON split_transactions(transaction_id);
CREATE INDEX idx_split_participants_user ON split_participants(user_id);
CREATE INDEX idx_split_participants_split ON split_participants(split_transaction_id);

-- 12. Create function to get user's total owed/owing
